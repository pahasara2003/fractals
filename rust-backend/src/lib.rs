...
#[wasm_bindgen]
pub fn is_julia(x: i32, y: i32,x1:f64,y1:f64,scale:f64,cx:f64,cy:f64,max_iter:usize) -> i32 {
    let mut zx = scale*2.0 * (y as f64 + y1) / (700.0);
    let mut zy = scale*2.0 * (x as f64 + x1) / (700.0);
...
#[wasm_bindgen]
pub fn generate_julia(x0:f64,y0:f64,scale:f64,screen_w:i32,screen_h:i32,max_iter:usize,cx:f64,cy:f64) -> Box<[u8]> {
    let (y_min, y_max) = (-screen_h/2, screen_h/2);
    let (x_min, x_max) = (-screen_w/2, screen_w/2);
    ...
    for y in y_min..y_max {
        for x in x_min..x_max {
            let state = is_julia(x as i32, y as i32,x0,y0,scale,cx,cy,max_iter);
            let index = ((y - y_min) * (x_max - x_min) + (x - x_min)) as usize;
            out_vec[index] = state as u8;
        }
    }
