...
#[wasm_bindgen]
pub fn is_mandelbrot(x: i32, y: i32,x1:f64,y1:f64,scale:f64) -> i32 {
    let y0 = scale*2.0 * (y as f64) / (700.0)+y1;
    let x0 = scale*2.0 * (x as f64) / (700.0)+x1;
...
#[wasm_bindgen]
pub fn generate_mandelbrot(x0:f64,y0:f64,scale:f64) -> Box<[u8]> {
    ...
    for y in y_min..y_max {
        for x in x_min..x_max {
            let state = is_mandelbrot(x as i32, y as i32,x0,y0,scale);
